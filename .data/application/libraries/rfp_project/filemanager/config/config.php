...
require_once($_SERVER['DOCUMENT_ROOT'] . "/.data/application/configSite.php");
...
function getAccessKey() {
	...
	require_once('include/AesHelper.class.php');
	$security = new AesHelper();

	if(isset($_GET['key'])) {
		// $dataArr = explode('&',file_get_contents('php://input'));
		$dataArr = explode('&', $_SERVER['QUERY_STRING']);
		foreach($dataArr as $key=>$value) {
			$aux = explode('=', $value);
			$_GET[$aux[0]] = isset($aux[1]) ? $aux[1] : "" ;
		}


		$aesKey = explode("}-()", $_GET['akey']);
		$aesKeyMd5 = md5($aesKey[0]);


		$decoded = rawurldecode(urldecode($_GET['key']));
		$data = $security->decrypt($decoded , $aesKeyMd5 , $aesKeyMd5 );


		$data = explode("}:@", $data);
		$access_key = md5($data[0]."}:@" . RFP_CIPHER )."}-()".$aesKey[1];
...
$config = array(
	...
	*/
	...
	'access_keys' => array( getAccessKey()),
	...
	*/
	...
	//**********************
	'ext_img'                                 => array( 'jpg', 'jpeg', 'png', 'gif' ), //Secure Images

	'ext_file'                                => array(  'pdf','txt', 'html', 'css' ), //Secure Files
	...
	*******************/
	'aviary_active'                           => true,
	'aviary_apiKey'                           => "da94624502024d91a8bc212a1a4f8874",
	'aviary_language'                         => "fa",
	'aviary_theme'                            => "light",
	'aviary_tools'                            => "all",
	'aviary_maxSize'                          => "1400",
	...
	'hidden_folders'                          => array('JorchinFilemanager' , 'jorchinContent' , 'assets' , 'preview'),
