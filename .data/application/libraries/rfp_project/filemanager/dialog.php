...
foreach($dataArr as $key=>$value) {
	$aux = explode('=', $value);
	$_GET[$aux[0]] = isset($aux[1]) ? $aux[1] : null ;
}
...
if (USE_ACCESS_KEYS == TRUE) {
	...
	if (!in_array($_GET['akey'], $access_keys)) {
	    echo "sss".$_GET['akey']."<br/>".$access_keys[0];
		die('Access Denied!');
	}
}
...
$get_params = array(
	...
	'key' => (isset($_GET['key']) && $_GET['key'] != '' ? $decoded : '')
...
$get_params = "akey=". (isset($_GET['akey']) && $_GET['akey'] != '' ? urldecode($_GET['akey']) : 'key')."&".http_build_query($get_params);
?>
<!DOCTYPE html>
